...
def filter_singular_models(df_scores):
...
def calculate_total_proportional_score(df_scores, criteria_dict):
...
def include_score_ranks(df_scores, criteria_dict):
    for init_routine in df_scores.init_routine.unique():
        for dataset in df_scores.dataset.unique():
            idxs = np.logical_and(df_scores.init_routine == init_routine, df_scores.dataset == dataset)
